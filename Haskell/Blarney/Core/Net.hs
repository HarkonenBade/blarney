...
-- | Tell if a 'Net' is a root 'Net' that can be optimized away, with either
--   * only zero-width inputs and no outputs
--   * only zero-width outputs and no inputs
isUselessRootNet Net{ netPrim = prim }
  | primIsRoot prim && (not $ primDontKill prim) =
    (all (\(_, w) -> w == 0)  ins && length outs == 0) ||
    (all (\(_, w) -> w == 0) outs && length  ins == 0)
  | otherwise = False
...
ignoreZeroWidthNet nl = do
  ...
  forM_ [i | x@(i, Just n) <- pairs, isUselessRootNet n] $ \idx -> do
...
eliminateDeadNet nl = do
  ...
  forM_ [(a,b) | x@(a, Just b) <- pairs] $ \(idx, net) -> do
    ...
    when (refCnt == 0 && (not . isPrimOutput) net
                      && (not . netDontKill)  net) $ do
      writeArray nl idx Nothing
      writeIORef changed True
  -- finish pass
  -- DEBUG HELP -- x <- readIORef changed
  ...
  readIORef changed
  where isPrimOutput Net{netPrim=Output _ _} = True
        isPrimOutput _ = False
